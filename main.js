...
"use strict";
module.exports = __webpack_require__.p + "bloat.png";

/***/ },

/***/ "./classes/ImageLoader.ts"
/*!********************************!*\
  !*** ./classes/ImageLoader.ts ***!
  \********************************/
...
// ============================================================================
class ImageLoader {
    constructor(targetSize = _constants__WEBPACK_IMPORTED_MODULE_1__.IMAGE_SIZE) {
        this.targetSize = targetSize;
        this.images = new Map();
        this.loadPromises = [];
    }
    loadAll(config, onLog) {
        for (const [key, url] of Object.entries(config)) {
            const promise = this.loadImage(url, key, onLog);
            this.loadPromises.push(promise);
        }
    }
    loadImage(url, key, onLog) {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                if (ctx) {
                    ...
                    this.images.set(key, {
                        str: alt1_base__WEBPACK_IMPORTED_MODULE_0__.encodeImageString(data),
    ...
    getMultiple(keys) {
        return keys.map(k => this.get(k)).filter((img) => !!img);
    }
}
...
class OverlayRenderer {
    constructor(imageLoader, settings) {
        this.imageLoader = imageLoader;
        this.settings = settings;
    }
    getPosition() {
        const { overlayOffsetX, overlayOffsetY, overlayScale } = this.settings();
        const scaledBarWidth = Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.BAR_WIDTH * overlayScale);
        const scaledBarHeight = Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.BAR_HEIGHT * overlayScale);
        // bar offset only scales down, not up (since images can't scale)
        const scaledBarYOffset = Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.BAR_Y_OFFSET * Math.min(1, overlayScale));
    ...
    getScaledImageGap() {
        ...
        return Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.IMAGE_GAP * scale);
    }
    getScaledBarHeight() {
        return Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.BAR_HEIGHT * this.settings().overlayScale);
    }
    getScaledTextSize() {
        return Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.TEXT_FONT_SIZE * this.settings().overlayScale);
    }
    drawImages(images, pos, duration) {
        if (!this.settings().showImage || images.length === 0)
            return;
        ...
        if (images.length === 1) {
            const img = images[0];
            alt1.overLayImage(Math.round(pos.centerX - img.width / 2), Math.round(pos.centerY - img.height - scaledGap), img.str, img.width, duration);
        }
        else {
            // multiple images side by side
            const totalWidth = images.reduce((sum, img) => sum + img.width, 0) +
                (images.length - 1) * _constants__WEBPACK_IMPORTED_MODULE_1__.MULTI_IMAGE_GAP;
            let xPos = Math.round(pos.centerX - totalWidth / 2);
            for (const img of images) {
                alt1.overLayImage(xPos, Math.round(pos.centerY - img.height - scaledGap), img.str, img.width, duration);
                xPos += img.width + _constants__WEBPACK_IMPORTED_MODULE_1__.MULTI_IMAGE_GAP;
            }
        }
    }
    drawText(text, color, pos, duration) {
        const scaledGap = this.getScaledImageGap();
        const yPos = this.settings().showImage ? pos.centerY + scaledGap : pos.centerY;
        alt1.overLayTextEx(text, color, this.getScaledTextSize(), pos.centerX, yPos, duration, "Arial", true, true);
    }
    drawProgressBar(progress, color, pos, duration) {
        if (!this.settings().showProgressBar)
            return;
        const scaledBarHeight = this.getScaledBarHeight();
        const scaledBarWidth = Math.round(_constants__WEBPACK_IMPORTED_MODULE_1__.BAR_WIDTH * this.settings().overlayScale);
        // background
        alt1.overLayLine(alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(50, 50, 50), scaledBarHeight, pos.barLeft, pos.barCenterY, pos.barRight, pos.barCenterY, duration);
        // foreground
        const filledWidth = Math.round(scaledBarWidth * progress);
        if (filledWidth > 0) {
            alt1.overLayLine(color, scaledBarHeight, pos.barLeft, pos.barCenterY, pos.barLeft + filledWidth, pos.barCenterY, duration);
        }
    }
    drawFinalMessage(imageKeys, pos) {
        alt1.overLaySetGroup(_constants__WEBPACK_IMPORTED_MODULE_1__.OVERLAY_GROUPS.FINAL);
        const images = this.imageLoader.getMultiple(imageKeys);
        this.drawImages(images, pos, _constants__WEBPACK_IMPORTED_MODULE_1__.FINAL_MESSAGE_DURATION);
        const scaledGap = this.getScaledImageGap();
        const yPos = this.settings().showImage ? pos.centerY + scaledGap : pos.centerY;
        alt1.overLayTextEx("NOW!", alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(255, 0, 0), Math.round(this.getScaledTextSize() + 10 * this.settings().overlayScale), pos.centerX, yPos, _constants__WEBPACK_IMPORTED_MODULE_1__.FINAL_MESSAGE_DURATION, "Arial", true, true);
    }
    clearAll() {
        alt1.overLayClearGroup(_constants__WEBPACK_IMPORTED_MODULE_1__.OVERLAY_GROUPS.TEXT);
        alt1.overLayClearGroup(_constants__WEBPACK_IMPORTED_MODULE_1__.OVERLAY_GROUPS.BAR);
    }
    drawDebugPreview() {
        ...
        const sequence = _constants__WEBPACK_IMPORTED_MODULE_1__.TIMER_SEQUENCES[settings.combatStyle];
        const firstStep = sequence[0];
        const images = this.imageLoader.getMultiple(firstStep.imageKeys);
        // static progress at 100%
        const progress = 1.0;
        const color = (0,_utils__WEBPACK_IMPORTED_MODULE_2__.getProgressColor)(progress);
        alt1.overLaySetGroup(_constants__WEBPACK_IMPORTED_MODULE_1__.OVERLAY_GROUPS.DEBUG);
        alt1.overLayClearGroup(_constants__WEBPACK_IMPORTED_MODULE_1__.OVERLAY_GROUPS.DEBUG);
        // draw actual image(s) with scaling
        this.drawImagesForDebug(images, pos, duration);
        // draw static countdown text with scaling
        const timeText = `${(0,_utils__WEBPACK_IMPORTED_MODULE_2__.formatTime)(firstStep.durationMs, settings.useTicks)}s`;
        const scaledGap = this.getScaledImageGap();
        const yPos = settings.showImage ? pos.centerY + scaledGap : pos.centerY;
        alt1.overLayTextEx(timeText, color, this.getScaledTextSize(), pos.centerX, yPos, duration, "Arial", true, true);
        ...
        if (settings.showProgressBar) {
            ...
            alt1.overLayLine(alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(50, 50, 50), scaledBarHeight, pos.barLeft, pos.barCenterY, pos.barRight, pos.barCenterY, duration);
            // foreground (full bar)
            alt1.overLayLine(color, scaledBarHeight, pos.barLeft, pos.barCenterY, pos.barRight, pos.barCenterY, duration);
        }
    }
    drawImagesForDebug(images, pos, duration) {
...
/***/ "./classes/P6ClonesTimingPlugin.ts"
/*!*****************************************!*\
  !*** ./classes/P6ClonesTimingPlugin.ts ***!
  \*****************************************/
(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   P6ClonesTimingPlugin: () => (/* binding */ P6ClonesTimingPlugin)
/* harmony export */ });
/* harmony import */ var alt1_base__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! alt1/base */ "../node_modules/alt1/dist/base/index.js");
/* harmony import */ var alt1_base__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(alt1_base__WEBPACK_IMPORTED_MODULE_0__);
...
/* harmony import */ var _ImageLoader__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./ImageLoader */ "./classes/ImageLoader.ts");
/* harmony import */ var _OverlayRenderer__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./OverlayRenderer */ "./classes/OverlayRenderer.ts");
/* harmony import */ var _SettingsManager__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./SettingsManager */ "./classes/SettingsManager.ts");
/* harmony import */ var _TimerController__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./TimerController */ "./classes/TimerController.ts");
...
class P6ClonesTimingPlugin {
    constructor() {
        ...
        this.imageLoader = new _ImageLoader__WEBPACK_IMPORTED_MODULE_3__.ImageLoader();
        this.settings = new _SettingsManager__WEBPACK_IMPORTED_MODULE_5__.SettingsManager({
            useTicks: document.getElementById("use-ticks"),
            showImage: document.getElementById("show-image"),
            showProgressBar: document.getElementById("show-progress-bar"),
            compensateForPing: document.getElementById("compensate-ping"),
            debugMode: document.getElementById("debug-mode"),
            combatStyle: document.querySelectorAll("input[name=\"combat-style\"]"),
            overlayOffsetX: document.getElementById("overlay-offset-x"),
            overlayOffsetY: document.getElementById("overlay-offset-y"),
            overlayScale: document.getElementById("overlay-scale"),
            output: this.outputElement
        });
        this.renderer = new _OverlayRenderer__WEBPACK_IMPORTED_MODULE_4__.OverlayRenderer(this.imageLoader, () => this.settings.current);
        this.timer = new _TimerController__WEBPACK_IMPORTED_MODULE_6__.TimerController(this.renderer, this.imageLoader, () => this.settings.current, () => this.log("⏱️ Timer sequence complete!"));
        this.chatReader = new (alt1_chatbox__WEBPACK_IMPORTED_MODULE_1___default())();
        // add NPC dialogue colors to the reader
        this.chatReader.readargs.colors.push(alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(_constants__WEBPACK_IMPORTED_MODULE_2__.NAME_RGB[0], _constants__WEBPACK_IMPORTED_MODULE_2__.NAME_RGB[1], _constants__WEBPACK_IMPORTED_MODULE_2__.NAME_RGB[2]), // NPC name cyan
        alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(_constants__WEBPACK_IMPORTED_MODULE_2__.TEXT_RGB[0], _constants__WEBPACK_IMPORTED_MODULE_2__.TEXT_RGB[1], _constants__WEBPACK_IMPORTED_MODULE_2__.TEXT_RGB[2]), // NPC text green
        alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(_constants__WEBPACK_IMPORTED_MODULE_2__.WHITE_RGB[0], _constants__WEBPACK_IMPORTED_MODULE_2__.WHITE_RGB[1], _constants__WEBPACK_IMPORTED_MODULE_2__.WHITE_RGB[2]), // white text
        alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(_constants__WEBPACK_IMPORTED_MODULE_2__.PUB_BLUE[0], _constants__WEBPACK_IMPORTED_MODULE_2__.PUB_BLUE[1], _constants__WEBPACK_IMPORTED_MODULE_2__.PUB_BLUE[2]) // public chat blue
        );
        this.init();
    }
    init() {
        this.settings.load();
        this.settings.setupListeners((msg) => this.log(msg), () => this.onOverlaySettingsChange());
        this.imageLoader.loadAll(_constants__WEBPACK_IMPORTED_MODULE_2__.IMAGE_CONFIG, (msg) => this.log(msg));
        ...
        if (alt1_base__WEBPACK_IMPORTED_MODULE_0__.hasAlt1) {
    ...
    onOverlaySettingsChange() {
        ...
        if (this.previewShowing && alt1_base__WEBPACK_IMPORTED_MODULE_0__.hasAlt1 && alt1.rsLinked) {
            this.renderer.drawDebugPreview();
        }
    }
    checkAlt1Status() {
    ...
    initializeOverlaySettings() {
        if (this.slidersUpdated || !alt1.rsWidth || !alt1.rsHeight)
            return;
    ...
    startChatReading() {
        ...
        this.chatInterval = window.setInterval(() => {
            this.readChatbox();
        }, _constants__WEBPACK_IMPORTED_MODULE_2__.TICK_MS);
    }
    readChatbox() {
        if (!alt1_base__WEBPACK_IMPORTED_MODULE_0__.hasAlt1 || !alt1.permissionPixel)
            return;
        if (!alt1.rsLinked) {
        ...
        this.initializeOverlaySettings();
        try {
            const img = alt1_base__WEBPACK_IMPORTED_MODULE_0__.captureHoldFullRs();
            if (!img) {
                if (this.findAttempts % 10 === 0) {
    ...
    findChatbox(img) {
        ...
        if (this.chatReader.pos) {
            ...
            const mainbox = this.chatReader.pos.mainbox;
            if (mainbox?.rect) {
                const { x, y, width, height } = mainbox.rect;
                this.log(`Chatbox at: x=${x}, y=${y}, w=${width}, h=${height}`);
                alt1.overLayRect(alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(0, 255, 0), x, y, width, height, 2000, 2);
            }
        }
        else {
            this.findAttempts++;
    ...
    processChatLines(img) {
        ...
        if (newLines.length > 0) {
            const triggered = newLines.some(line => _constants__WEBPACK_IMPORTED_MODULE_2__.CLONE_TRIGGERS.some(trigger => line.toUpperCase().includes(trigger)));
            if (triggered) {
    ...
    startTimer() {
        ...
        const sequence = _constants__WEBPACK_IMPORTED_MODULE_2__.TIMER_SEQUENCES[settings.combatStyle];
...
class SettingsManager {
    constructor(elements) {
        ...
        this.settings = { ..._constants__WEBPACK_IMPORTED_MODULE_0__.DEFAULT_SETTINGS };
        this.onOverlayChange = null;
    }
    load() {
        try {
            const saved = localStorage.getItem(_constants__WEBPACK_IMPORTED_MODULE_0__.SETTINGS_STORAGE_KEY);
            if (saved) {
                const parsed = JSON.parse(saved);
                this.settings = { ..._constants__WEBPACK_IMPORTED_MODULE_0__.DEFAULT_SETTINGS, ...parsed };
            }
        }
        catch (error) {
    ...
    save() {
        try {
            localStorage.setItem(_constants__WEBPACK_IMPORTED_MODULE_0__.SETTINGS_STORAGE_KEY, JSON.stringify(this.settings));
        }
        catch (error) {
    ...
    applyToUI() {
        ...
        if (useTicks)
            useTicks.checked = this.settings.useTicks;
        if (showImage)
            showImage.checked = this.settings.showImage;
        if (showProgressBar)
            showProgressBar.checked = this.settings.showProgressBar;
        if (compensateForPing)
            compensateForPing.checked = this.settings.compensateForPing;
        if (debugMode)
    ...
    updateSliderMaxValues(maxX, maxY) {
    ...
    initializeDefaultPosition(centerX, centerY) {
    ...
    setupListeners(onLog, onOverlayChange) {
        ...
        combatStyle?.forEach(radio => {
            radio.addEventListener("change", () => {
                this.update("combatStyle", radio.value);
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   TimerController: () => (/* binding */ TimerController)
/* harmony export */ });
/* harmony import */ var _constants__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../constants */ "./constants.ts");
...
class TimerController {
    constructor(renderer, imageLoader, settings, onComplete) {
    ...
    start(steps) {
        ...
        const textRefreshRate = settings.useTicks ? _constants__WEBPACK_IMPORTED_MODULE_0__.TICK_MS : 16;
        ...
        const drawTextOverlay = () => {
            ...
            const color = (0,_utils__WEBPACK_IMPORTED_MODULE_1__.getProgressColor)(progress);
            const images = this.imageLoader.getMultiple(current.step.imageKeys);
            alt1.overLaySetGroup(_constants__WEBPACK_IMPORTED_MODULE_0__.OVERLAY_GROUPS.TEXT);
            alt1.overLayClearGroup(_constants__WEBPACK_IMPORTED_MODULE_0__.OVERLAY_GROUPS.TEXT);
            this.renderer.drawImages(images, pos, textRefreshRate + 100);
            this.renderer.drawText(`${(0,_utils__WEBPACK_IMPORTED_MODULE_1__.formatTime)(current.remainingMs, this.settings().useTicks)}s`, color, pos, textRefreshRate + 100);
            return true;
        };
        ...
        const drawProgressBar = () => {
            const elapsed = Date.now() - startTime;
            const current = getCurrentStep(elapsed);
            alt1.overLaySetGroup(_constants__WEBPACK_IMPORTED_MODULE_0__.OVERLAY_GROUPS.BAR);
            alt1.overLayClearGroup(_constants__WEBPACK_IMPORTED_MODULE_0__.OVERLAY_GROUPS.BAR);
            if (!current) {
            ...
            const progress = current.remainingMs / current.step.durationMs;
            const color = (0,_utils__WEBPACK_IMPORTED_MODULE_1__.getProgressColor)(progress);
            this.renderer.drawProgressBar(progress, color, pos, _constants__WEBPACK_IMPORTED_MODULE_0__.PROGRESS_REFRESH_RATE + 50);
            return true;
        };
        // initial draw
        drawTextOverlay();
        ...
        }, _constants__WEBPACK_IMPORTED_MODULE_0__.PROGRESS_REFRESH_RATE);
    }
    finish(lastImageKeys, pos) {
...
/* harmony export */   OverlayRenderer: () => (/* reexport safe */ _OverlayRenderer__WEBPACK_IMPORTED_MODULE_1__.OverlayRenderer),
/* harmony export */   P6ClonesTimingPlugin: () => (/* reexport safe */ _P6ClonesTimingPlugin__WEBPACK_IMPORTED_MODULE_2__.P6ClonesTimingPlugin),
/* harmony export */   SettingsManager: () => (/* reexport safe */ _SettingsManager__WEBPACK_IMPORTED_MODULE_3__.SettingsManager),
/* harmony export */   TimerController: () => (/* reexport safe */ _TimerController__WEBPACK_IMPORTED_MODULE_4__.TimerController)
/* harmony export */ });
/* harmony import */ var _ImageLoader__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./ImageLoader */ "./classes/ImageLoader.ts");
/* harmony import */ var _OverlayRenderer__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./OverlayRenderer */ "./classes/OverlayRenderer.ts");
/* harmony import */ var _P6ClonesTimingPlugin__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./P6ClonesTimingPlugin */ "./classes/P6ClonesTimingPlugin.ts");
/* harmony import */ var _SettingsManager__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./SettingsManager */ "./classes/SettingsManager.ts");
/* harmony import */ var _TimerController__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./TimerController */ "./classes/TimerController.ts");
// barrel export for classes
...
/* harmony export */   BAR_WIDTH: () => (/* binding */ BAR_WIDTH),
/* harmony export */   BAR_Y_OFFSET: () => (/* binding */ BAR_Y_OFFSET),
/* harmony export */   CLONE_TRIGGERS: () => (/* binding */ CLONE_TRIGGERS),
/* harmony export */   DEFAULT_SETTINGS: () => (/* binding */ DEFAULT_SETTINGS),
/* harmony export */   FINAL_MESSAGE_DURATION: () => (/* binding */ FINAL_MESSAGE_DURATION),
/* harmony export */   IMAGE_CONFIG: () => (/* binding */ IMAGE_CONFIG),
/* harmony export */   IMAGE_GAP: () => (/* binding */ IMAGE_GAP),
/* harmony export */   IMAGE_SIZE: () => (/* binding */ IMAGE_SIZE),
/* harmony export */   MULTI_IMAGE_GAP: () => (/* binding */ MULTI_IMAGE_GAP),
/* harmony export */   NAME_RGB: () => (/* binding */ NAME_RGB),
/* harmony export */   OVERLAY_GROUPS: () => (/* binding */ OVERLAY_GROUPS),
/* harmony export */   PROGRESS_REFRESH_RATE: () => (/* binding */ PROGRESS_REFRESH_RATE),
/* harmony export */   PUB_BLUE: () => (/* binding */ PUB_BLUE),
/* harmony export */   SETTINGS_STORAGE_KEY: () => (/* binding */ SETTINGS_STORAGE_KEY),
/* harmony export */   TEXT_FONT_SIZE: () => (/* binding */ TEXT_FONT_SIZE),
/* harmony export */   TEXT_RGB: () => (/* binding */ TEXT_RGB),
/* harmony export */   TICK_MS: () => (/* binding */ TICK_MS),
/* harmony export */   TIMER_SEQUENCES: () => (/* binding */ TIMER_SEQUENCES),
/* harmony export */   WHITE_RGB: () => (/* binding */ WHITE_RGB)
/* harmony export */ });
/* harmony import */ var _icon_png__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./icon.png */ "./icon.png");
/* harmony import */ var _omni_png__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./omni.png */ "./omni.png");
/* harmony import */ var _nami_png__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./nami.png */ "./nami.png");
/* harmony import */ var _invoke_png__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./invoke.png */ "./invoke.png");
/* harmony import */ var _threads_png__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./threads.png */ "./threads.png");
/* harmony import */ var _volley_png__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./volley.png */ "./volley.png");
/* harmony import */ var _t90_png__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./t90.png */ "./t90.png");
/* harmony import */ var _eof_png__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./eof.png */ "./eof.png");
/* harmony import */ var _bloat_png__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./bloat.png */ "./bloat.png");
// image imports
...
const IMAGE_SIZE = 64;
const IMAGE_CONFIG = {
    magic: _icon_png__WEBPACK_IMPORTED_MODULE_0__,
    omni: _omni_png__WEBPACK_IMPORTED_MODULE_1__,
    nami: _nami_png__WEBPACK_IMPORTED_MODULE_2__,
    invoke: _invoke_png__WEBPACK_IMPORTED_MODULE_3__,
    threads: _threads_png__WEBPACK_IMPORTED_MODULE_4__,
    volley: _volley_png__WEBPACK_IMPORTED_MODULE_5__,
    t90: _t90_png__WEBPACK_IMPORTED_MODULE_6__,
    eof: _eof_png__WEBPACK_IMPORTED_MODULE_7__,
    bloat: _bloat_png__WEBPACK_IMPORTED_MODULE_8__
};
// ============================================================================
// TIMER SEQUENCES
// ============================================================================
const TIMER_SEQUENCES = {
    magic: [
...
"use strict";
module.exports = __webpack_require__.p + "eof.png";
...
"use strict";
module.exports = __webpack_require__.p + "invoke.png";
...
"use strict";
module.exports = __webpack_require__.p + "nami.png";
...
"use strict";
module.exports = __webpack_require__.p + "omni.png";
...
"use strict";
module.exports = __webpack_require__.p + "t90.png";
...
"use strict";
module.exports = __webpack_require__.p + "threads.png";
...
function getProgressColor(progress) {
    if (progress > 0.5)
        return alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(0, 255, 0); // green
    if (progress > 0.2)
        return alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(255, 255, 0); // yellow
    return alt1_base__WEBPACK_IMPORTED_MODULE_0__.mixColor(255, 0, 0); // red
}
function formatTime(ms, useTicks) {
    if (useTicks) {
...
"use strict";
module.exports = __webpack_require__.p + "volley.png";
...
/* harmony import */ var _classes__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./classes */ "./classes/index.ts");
...
document.addEventListener("DOMContentLoaded", () => {
    new _classes__WEBPACK_IMPORTED_MODULE_3__.P6ClonesTimingPlugin();
});
